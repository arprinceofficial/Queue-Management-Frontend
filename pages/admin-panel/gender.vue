<script setup>
    useHead({ title: 'Admin | Gender' });
    definePageMeta({ middleware: ['auth-admin'], layout: 'admin' });
    const { 
        $api_admin_gender_list,
        $api_admin_gender_delete,
    } = useNuxtApp();
    ...
    const receivedData = (data) => {
        ...
        admin_gender_list.value.push(data) :
        admin_gender_list.value = admin_gender_list.value.map(item => item.id == data.id ? data : item); 
    }
    const addGender = () => {
        data.value = {};
        modal_title.value = 'Add';
    ...
    const deleteGender = async (id) => {
        ...
        try{
            ...
            const getData = await $fetchAdmin($api_admin_gender_delete, {
                method: 'POST',
                body: {id: id},
            });
            ...
            <div class="flex h-full">
                ...
                <div v-else class="h-full w-full overflow-auto">
                    <div class="w-full flex justify-end pt-5 px-8">
                        <button @click="addGender" class="bg-[#0083C4] text-white py-2 px-4 rounded-lg ml-4 mb-4">
                    ...
                    <div class="px-6 pb-2 flex flex-col justify-between w-full">
                        <div class="mt-4 border border-gray-200 rounded-lg">
                            <div class="border-b border-gray-200">
                                <h4 class="text-[18px] font-semibold dark:text-gray-200 py-2 px-4">Gender List</h4>
                            </div>
                            <div class="p-4">
                                <div class="custom_table overflow-auto border-b border-gray-200">
                                    <table class="table border table-auto">
                                        ...
                                        <tbody class="">
                                            <tr v-for="(item, index) in admin_gender_list" :key="index">
                                                ...
                                                <td>
                                                    <div class="flex justify-center items-center gap-2">
                                                        ...
                                                        <div @click="deleteGender(item.id)"
