<script setup>
    useHead({ title: 'Admin | Agent User' });
    ...
    const deleteAgentUser = async (id) => {
        ...
        try{
            ...
            const getData = await $fetchAdmin($api_admin_agent_user_delete, {
                method: 'POST',
                body: {id: id},
            });
            if (getData.status == true) {
                admin_agent_user_list.value = admin_agent_user_list.value.filter(item => item.id != id);
            }
        } catch(e){
            console.log('Get Message',e.message);
        } finally {
...
<template>
    <div class="h-[calc(100vh-76px)]">
                                            ...
                                            <tr v-for="(item, index) in admin_agent_user_list" :key="index">
                                                ...
                                                <td>
                                                    <div class="flex justify-center items-center gap-2">
                                                        <button @click="editAgentUser(item)" 
                                                        ...
                                                        <div @click="deleteAgentUser(item.id)"
        ...
        <AdminAgentUserAddEdit 
            :isOpen="is_open_modal"
            :title="modal_title"
            :data="data"
            @add_AgentUser="receivedData"
            @cancel="cancelModal"
        />
