<script setup>
    useHead({ title: 'Admin | Service' });
    definePageMeta({ middleware: ['auth-admin'], layout: 'admin' });
    const { 
        $api_admin_service_list,
    ...
    const deleteService = async (id) => {
        ...
        try{
            ...
            const getData = await $fetchAdmin($api_admin_service_delete, {
                method: 'POST',
                body: {id: id},
            });
            response_modal.value = getData;
            if (getData.status == true) {
                admin_service_list.value = admin_service_list.value.filter(item => item.id != id);
            }
        } catch(e){
            console.log('Get Message',e.message);
