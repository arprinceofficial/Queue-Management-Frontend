...
const current_pg = ref(data.value.current_page ? data.value.current_page : 1)
...
<template>
    <div class="flex items-center justify-between border-t border-gray-200 py-3">
        <div class="flex flex-1 justify-between sm:hidden">
            <div class="relative inline-flex items-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 cursor-pointer"
                @click="go_previous()">Previous</div>
            <div class="relative ml-3 inline-flex items-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 cursor-pointer"
        ...
        <div class="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
            ...
            <div>
                <nav class="isolate inline-flex -space-x-px rounded-md shadow-sm" aria-label="Pagination">
                    <div class="relative inline-flex items-center rounded-l-md border border-gray-300 bg-white px-2 py-2 text-sm font-medium text-gray-500 hover:bg-gray-50 focus:z-20 cursor-pointer"
                        @click="go_previous()">
                    ...
                    <template v-if="pg_display_limit >= data.last_page">
                        <div :aria-current="[data.current_page == pg ? 'page' : '']"
                            :class="['relative z-10 inline-flex items-center border px-4 py-2 text-sm font-medium focus:z-20 cursor-pointer', data.current_page == pg ? 'border-green-500 bg-green-50 text-green-600' : '']"
                    ...
                    <template v-else>
                        ...
                        <template v-if="first_page_display">
                            <div :aria-current="[data.current_page == 1 ? 'page' : '']"
                                :class="['relative z-10 inline-flex items-center border px-4 py-2 text-sm font-medium focus:z-20 cursor-pointer', data.current_page == 1 ? 'border-green-500 bg-green-50 text-green-600' : '']"
                        ...
                        <template v-if="last_page_display">
                            ...
                            <div :aria-current="[data.current_page == data.last_page ? 'page' : '']"
                                :class="['relative z-10 inline-flex items-center border px-4 py-2 text-sm font-medium focus:z-20 cursor-pointer', data.current_page == data.last_page ? 'border-green-500 bg-green-50 text-green-600' : '']"
                                @click="load_content(data.last_page)" v-html="pg_num_display(data.last_page)"></div>
                        </template>

                    </template>

                    <div class="relative inline-flex items-center rounded-r-md border border-gray-300 bg-white px-2 py-2 text-sm font-medium text-gray-500 hover:bg-gray-50 focus:z-20 cursor-pointer"
                        @click="go_next()">
