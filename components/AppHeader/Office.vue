            ...
            <div class="flex justify-between h-full">
                <div class="flex">
                    ...
                    <div class="shrink-0 flex items-center">
                        <NuxtLink href="/">
                            <ApplicationLogo :width="'220px'" :height="'220px'" />
                        </NuxtLink>
                    </div>
                </div>
                            ...
                            <template #trigger>
                                <span class="inline-flex rounded-md">
                                    <button type="button"
                                        ...
                                        <img class="h-8 w-8 rounded-full object-cover" :src="office_user?.data?.image ?? '/img/profile-image.png'" :class="!office_user?.data?.image ? 'bg-gray-200 p-1' : ''"
                                        alt="image" />
                                        <span class="pr-2 pl-2">{{ office_user?.data?.office_name }}</span>
                                    </button>
                                </span>
                            </template>
                                ...
                                <div class="py-2 border-b border-gray-200 dark:border-gray-600">
                                    <div class="px-4">
                                        <div class="font-medium text-sm text-gray-800 dark:text-gray-200">
                                            {{ office_user?.data?.office_name }}
                                        </div>
                                        <div class="font-medium text-sm text-gray-500">{{ office_user?.data?.user_name }}</div>
                                    </div>
                                </div>
